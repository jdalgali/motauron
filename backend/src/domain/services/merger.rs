use crate::domain::entities::{ListingStatus, MotorcycleListing};
use crate::domain::services::scorer;
...
pub struct MarketSummary {
    ...
    pub top_deals: Vec<MotorcycleListing>,
}
...
impl MarketSummary {
    pub fn print(&self) {
        ...
        println!();
        println!("  tracking  {} listings", self.total_tracked);
    }
}

pub fn merge_listings(
    mut db: HashMap<u64, MotorcycleListing>,
    scraped: Vec<MotorcycleListing>,
) -> (HashMap<u64, MotorcycleListing>, MarketSummary) {
    ...
    for category in &scraped_categories {
        ...
        scorer::score_category(&mut group);
    ...
    let mut all_active: Vec<MotorcycleListing> = db
        .values()
        .filter(|l| l.status == ListingStatus::Active)
        .cloned()
        .collect();
    all_active.sort_by(|a, b| b.price_score.cmp(&a.price_score));
    summary.top_deals = all_active.into_iter().take(5).collect();
